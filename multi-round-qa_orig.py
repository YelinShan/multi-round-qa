...
class RequestExecutor:
    def __init__(self, base_url: str, api_key: str, model: str):
        ...
        self.STATS_TOTAL_SEND = 0
        self.log_file = open("requests.jsonl", "w", encoding="utf-8")

    async def _async_launch_request(self, messages, max_tokens, extra_headers=None, extra_body=None):
        ...
        return Response(
            ...
            generation_time=time.time() - first_token_time if first_token_time is not None else None,
...
class UserSession:
    ...
    def _launch_new_request(self, timestamp: float, request_executor: RequestExecutor):
        ...
        if self.use_sharegpt:
            if self.start_with_gpt:
                ...
                if 2 * (self.question_id - 1) + 2 < len(self.sharegpt_data["conversations"]):
                    ...
                    # print(f"11111questionid: {self.sharegpt_data["id"]} {2 * (self.question_id - 1) + 2}")
                ...
                if 2 * (self.question_id - 1) + 1 < len(self.sharegpt_data["conversations"]):
                    # print(f"22222questionid: {self.sharegpt_data["id"]} {2 * (self.question_id - 1) + 1}")
        ...
        request_executor.launch_request(
            ...
            extra_body={"request_id": f"user_{self.user_config.user_id}-{self.sharegpt_data["id"]}", 
                        "min_tokens": max_tokens-1, 
                        "seed": 12345}  # 基于用户ID和问题ID的确定性seed
        )
...
class UserSessionManager:
    ...
    def _load_sharegpt_data(self):
        with open("ShareGPT_fixed4.json", "r", encoding="utf-8") as file:
        ...
        # 按照ID排序，确保每次加载的顺序一致
        self.sharegpt_data.sort(key=lambda x: x.get("id", ""))
        logger.info(f"There are {len(self.sharegpt_data)} users satisfying the requirements")
    ...
    def ProcessSummary(
        ...
        total_generation_tokens = df["generation_tokens"].dropna().sum()
        average_prefill_speed = total_prompt_tokens / total_time
        average_generation_speed = total_generation_tokens / total_time
        # 过滤掉generation_time为None的行
        df_valid = df.dropna(subset=["generation_time"])
        average_generation_speed_per_request = (
            df_valid["generation_tokens"] / df_valid["generation_time"]
        ).mean() if not df_valid.empty else 0.0
        average_ttft = df["ttft"].dropna().mean()
...
def main():
    ...
    logger.info(f"TOTALLY SEND {executor.STATS_TOTAL_SEND} requests.")
